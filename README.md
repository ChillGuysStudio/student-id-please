...
- **Storage per service:** We chose the database engines that best fit the data structure of each specific domain. Some services utilize multiple stores to separate live state from persistent history.
  ...
  - **Redis** acts as an in-memory datastore for the Discord DMs and Session services. It coordinates live WebSocket message broadcasting (via Pub/Sub) rather than owning the connection objects themselves, and provides distributed locks for concurrent case starts alongside fast lookups for active shift rosters.
