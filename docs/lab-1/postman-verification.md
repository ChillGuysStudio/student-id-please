# Player and Session Postman verification guide

The CPR contains separate Postman collections for Player and Session. The common Compose deployment and its database volumes will be delivered in a separate team PR. To start a service now, follow its private repository run guide: [Player](https://github.com/Tirppy/student-id-player-service/blob/dev/docs/running.md) or [Session](https://github.com/Tirppy/student-id-session-service/blob/dev/docs/running.md).
