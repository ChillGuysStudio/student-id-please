...
The published Lab 1 review image is [`tirppy/student-id-session-service:1.0.0-rc.2`](https://hub.docker.com/r/tirppy/student-id-session-service/tags). Follow the [private run guide](https://github.com/Tirppy/student-id-session-service/blob/feat/lab-1/session-service/docs/running.md) for standalone source or container setup. In the [CPR verification flow](../lab1-running.md), Session calls real Player and uses typed mocks for the unavailable teammate services. Those mocks return a representative published ruleset, distributed permissions, a stable snapshot ID, deterministic case IDs, and readiness responses. They do not generate applicant facts or decide admission outcomes. The Session collection supplies a `DecisionScored` fixture. The private repository README gives the Lab 1 Grade 3 run instructions.
