...
The public image is `maxnoragami/server-rules-service:<version>` ([published tags](https://hub.docker.com/r/maxnoragami/server-rules-service/tags)). To pin a release, set `IMAGE_TAG` in your shell, for example `export IMAGE_TAG=1.0.0`. Leave `IMAGE_TAG` unset to use `latest`, which changes with new releases.

The API listens on container port `8081`. Bind it to `127.0.0.1:8081` for a local check. On first startup with an empty database, the service seeds the CPR baseline rules as one published version and never overwrites admin data afterward.

Set these values in a local `.env`. Do not commit `.env` or tokens.

| Setting | Required value |
| --- | --- |
| `POSTGRES_DATABASE`, `POSTGRES_PORT`, `POSTGRES_USER`, `POSTGRES_PASSWORD` | Database name, host port, user, and password for the local PostgreSQL. |
| `SERVER_PORT` | Service port, defaults to `8081`. |
| `ADMIN_TOKEN`, `PLAYER_TOKEN`, `SESSION_TOKEN`, `MODERATION_TOKEN` | Static bearer tokens per caller role. Verified Player and service credentials replace them in a later lab. |
| `CURSOR_SECRET` | Secret for signing pagination cursors, at least 32 characters. Rotating it invalidates outstanding cursors. |
