...
This README retains the Lab 0 gameplay design. Applicant and Credential now implement standalone persistent CRUD for Lab 1; the other services and integrated gameplay flows remain planned.
...
- [Run Applicant and Credential CRUD](#run-applicant-and-credential-crud)
...
## Run Applicant and Credential CRUD

Both services use **Java 25**, **Spring Boot 4.1.1**, and the **Maven 3.9.16 Wrapper**, with separate MongoDB databases. Install JDK 25 and a Docker engine with Compose. Set `JAVA_HOME` to the JDK 25 installation. On macOS with Homebrew, this is typically `/opt/homebrew/opt/openjdk@25/libexec/openjdk.jdk/Contents/Home`.
...
Run `./mvnw spring-boot:run` in `applicant-service` and `credential-service` in separate terminals. Applicant listens on `127.0.0.1:8081`; Credential listens on `127.0.0.1:8082`. Each service README provides environment variables, request examples, and its full implemented CRUD contract:

- [Applicant setup and API](applicant-service/README.md#implemented-standalone-crud)
- [Credential setup and API](credential-service/README.md#implemented-standalone-crud)

| Service | Collection | Item identifier | Operations |
| --- | --- | --- | --- |
| Applicant | `/api/v1/applicants` | `case_id` | POST collection; GET collection/item; PUT/DELETE item |
| Credential | `/api/v1/credentials` | `credential_id` | POST collection; GET collection/item; PUT/DELETE item |

Creation returns `201` with the resource and `Location`; reads/updates return `200`; deletion returns `204`. PUT replaces mutable fields and never creates missing records. Applicant creation takes `session_id` and `claims`; updates accept only `claims`. Credential creation takes the documented credential fields except `credential_id`; updates exclude both IDs. Required nullable fields must be explicitly present. Identifiers are generated by the owning service and never changed by PUT. Credential listing can filter by `case_id`.

These additions follow the contract's JSON, pagination, error, and idempotency conventions. Successful mutations and replay records commit in one MongoDB transaction. Keys are scoped to method, path, and a fixed local caller, and retained after resource deletion. Cross-service references are currently format-checked only; applicant deletion never cascades into Credential.
