...
The published Lab 1 review image is [`tirppy/student-id-session-service:1.0.0-rc.2`](https://hub.docker.com/r/tirppy/student-id-session-service/tags). Follow the [private run guide](https://github.com/Tirppy/student-id-session-service/blob/e42ea58aebc58afb32654c3902a24f86b3e28309/docs/running.md) for standalone source or container setup and Postman verification. Session calls real Player and uses typed mocks for the unavailable teammate services. Those mocks return a representative published ruleset, distributed permissions, a stable snapshot ID, deterministic case IDs, and readiness responses. They do not generate applicant facts or decide admission outcomes. The Session collection supplies a `DecisionScored` fixture.
